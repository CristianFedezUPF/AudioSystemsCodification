...
"""
Functions here handle the UI for the different tasks and call
tasks_tkinter.py for the actual conversions when necessary.
"""


def resize_window(mainframe: Frame, width: int, height: int):
    root = mainframe.master  # master of mainframe is always root Tk()
    root.minsize(width, height)  # resize to fit all widgets


def init_window(mainframe: Frame):
    resize_window(mainframe, 550, 200)
...
def convert_res(mainframe: Frame):
    ...
    def call_task():
        tasks.convert_resolution(input_file.get(), resolution.get(), mainframe)

    b1 = ttk.Button(mainframe, text="Convert!", command=call_task)
    b1.grid(column=3, row=2, sticky=W)

    b2 = ttk.Button(mainframe, text="< Back",
                    command=partial(utils.reset_to_menu, mainframe))
    b2.grid(column=0, row=0, sticky=W, padx=20)


def convert_codec(mainframe: Frame):
    resize_window(mainframe, 630, 200)
    ...
    def call_task():
        tasks.convert_codec(input_file.get(), codec.get(), mainframe,
                            crf=crf.get())

    b2 = ttk.Button(lower_row, text="Convert!", command=call_task)
    b2.grid(column=2, row=2, sticky=W)


def stack_4_videos(mainframe: Frame):
    resize_window(mainframe, 950, 350)

    utils.clear_frame(mainframe)

    b1 = ttk.Button(mainframe, text="< Back",
                    command=partial(utils.reset_to_menu, mainframe))
    b1.grid(column=0, row=0, sticky=W, padx=20)
    ...
    def call_task():
        tasks.stack_videos(input1_entry.get(), input2_entry.get(),
                           input3_entry.get(), input4_entry.get(),
                           [tag.get() for tag in tags], output_entry.get(),
                           mainframe)

    b2 = ttk.Button(mainframe, text="Stack!", command=call_task)
    b2.grid(column=3, row=2, sticky=W)
